...
class TodoItemView extends View {
  ...
  _generateMarkup() {
    ...
    return `
      <li class="hover:bg-gray-100 cursor-pointer py-3 border-b" data-id='${
        this._data.id
      }'>
        <div class="grid grid-cols-6">
          ...
          <div class="task-body col-span-4">
            <h3 class="font-bold text-base">${this._data.title}</h3>
          ...
          <div id="delete-btn" class="justify-self-center self-center cursor-pointer group">
            <svg class="w-5 h-5">
              <use
                class="stroke-black/20 group-hover:stroke-black/60 stroke-2"
                href="${icons}#trash"
              ></use>
            </svg>
          </div>
...
export default new TodoItemView();
